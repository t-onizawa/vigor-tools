...
## 本番URLを検証するときの注意（2026-09-14追記）

本番URL（`https://vigorlab.net/...`）をブラウザで開いて検証する場合
（新規ページのHTTP 200確認、390×844/1440×900のレイアウト確認、
console error確認等）、URLに`?internal=1`（既存クエリがある場合は
`&internal=1`）を必ず付けてアクセスする。付けないと、その検証アクセス
が実際のGA4データとして計測されてしまう。

検証セッションは実行のたびにブラウザコンテキストがリセットされる
可能性が高いため、「一度だけ付ければよい」ではなく、**検証アクセスの
たびに毎回付ける**こと。対象はMATSURI・TOOLS問わず、本番URLをブラウザ
で開くすべての検証手順に適用する。
...
### note自動公開の例外（2026-09-14追記）

「今週のおすすめ祭り」タスク（VIGOR MATSURIの開催間近の祭りを紹介する
note記事）に限り、Founderが自動公開を明示的に許可している。このタスク
の指示書に「公開してよい」と明記されている場合のみ例外とし、他のnote
関連タスク（週次note下書き等）には適用しない。理由：一人称エッセイと
異なり、一次情報（data.js・research/{slug}.md）で確認できた事実のみを
扱う機械的な紹介文であるため。ただし実在する祭りの日程・開催状況を
扱うため、直前の中止・延期等の急な変更リスクは残る点に留意する。
