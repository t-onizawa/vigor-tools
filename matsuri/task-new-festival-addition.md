...
1. 下記「候補ストック」に一次情報確認済み・未追加の候補があれば、
   新規探索より先にそこから追加する（2026-09-18追加。詳細は後述）
2. 各地方で取りこぼした品質基準適合候補
3. 新たに公式情報が公開された候補
4. 過去に保留した候補
5. 上記1〜4だけでは候補が確保できない場合、都道府県単位で新規候補を
   ...
   **情報源を使い切ってから次へ進む（2026-09-18追加）：** 観光協会
   カレンダー等、複数の祭りが掲載された情報源を開いた場合は、1件
   採用できた時点で確認を打ち切らない。その情報源に掲載されている
   候補を、1回の上限5件に達するまで、またはその情報源に掲載された
   候補を確認し終えるまで、できる限りまとめて確認する。

   **1つの情報源で5件に届かない場合（2026-09-18追加）：** 同じ実行内
   で追加の情報源（別の市区町村・別の観光協会サイト等）を続けて確認
   する。5件確保できるまで探索を続けることを原則とし、途中で切り上げ
   てよいのは、質・停止条件を理由に候補を見送った場合、または実際に
   確認できる情報源（観光協会・自治体・神社庁等の一次情報に辿れる
   もの）を確認し尽くした場合に限る。「時間がかかるから」「ある程度
   確認したから」という理由だけで5件未満のまま打ち切ってはならない。
   目安として、最低5つの異なる市区町村・情報源を確認してもなお5件に
   届かない場合のみ、その回の結果として受け入れる（2026-09-18改訂：
   従来の目安「最低3つ」は打ち切りの言い訳に使われやすいため引き上げ
   た）。

   **候補が上限を超えて見つかった場合（2026-09-18追加）：** 1回の
   実行で、採用条件を満たすと確認できたが5件の上限に収まらなかった
   候補が見つかった場合は、見送らず下記「候補ストック」へ記録する。

品質基準・採用条件・停止条件は初期展開時と完全に同一のまま緩めない。
候補数を確保するための基準緩和・推測での採用は行わない。一次情報で
確認できない候補は見送る。上記の「情報源を使い切る」「複数情報源を
確認する」はいずれも探索範囲を広げるための指示であり、採用条件・
停止条件そのものを変更するものではない。
...
全国探索中に、一次情報で採用条件（本ファイル「採用条件」節）を満たす
と確認できたが、1回の実行の上限5件に収まらなかった候補を記録する
ストック。次回以降の実行では、新規探索より先にここから追加する
（前掲「全国完了後の実行」優先順位1番）。

Backlog.mdの該当節（`## 新規祭り 候補ストック`）に、以下の形式で
追記する。

```
- {祭り名}（{都道府県}{市区町村}）：{一次情報URL}、確認日{YYYY-MM-DD}、
  {簡単な確認済み事項（開催日・開催主体・見どころ等）}
```

ストックから追加した候補は、Backlog.mdの当該行を削除する。

ストックの候補を追加する際も、他の候補と同様に静的・ブラウザ検証を
実施する。ストックへ記録した時点の一次情報確認から時間が経っている
場合（目安8週間以上）は、状況が変わっていないか一次情報を再確認して
から追加する。
...
- 現在の対象エリア
- 追加件数
- 追加件数が5件未満の場合、確認した市区町村・情報源の数と一覧、
  5件に届かなかった具体的な理由（2026-09-18追加。「候補枯渇」
  「確認できる情報源を確認し尽くした」など正当な理由がある場合に
  限り、その理由と実際に確認した情報源数を明記する。確認した情報源
  が5未満のまま「見つからなかった」とだけ書くことは正当な理由に
  ならない）
- 追加した祭り
...
2026-09-18（1回の実行あたりの発見数を増やす改訂）
    全国深掘りフェーズの直近7回（2026-09-13〜09-18）が毎回1件のみの
    追加に留まっていたことをFounderが指摘。実行ログを確認したところ、
    9/17・9/18の2回連続で同一市（仙北市）の公式イベントカレンダーから
    1件ずつしか拾っておらず、情報源を見つけても1件採用した時点で
    確認を打ち切っていたことが判明した（日本全国の祭りの母集団自体が
    枯渇しているのではなく、1回の実行で使う情報源の数・情報源ごとに
    確認する候補の数が狭すぎたことが原因）。品質基準・採用条件・
    停止条件は一切変更せず、(1)情報源を見つけたら1件で打ち切らず
    その情報源に掲載された候補を上限5件まで、または情報源を確認し
    尽くすまでまとめて確認する、(2)1つの情報源で5件に届かない場合は
    同じ実行内で追加の情報源（目安：最低3つ）を続けて確認する、
    (3)上限に収まらなかった適合候補は見送らず「候補ストック」
    （Backlog.md新設）へ記録し次回優先的に追加する、の3点を追加した。

2026-09-18（探索効果の担保を強化：最低3つ→5つ、完了報告に理由必須化）
    「祭りが本当になくなるまで5件ずつ追加する運用にしたい」という
    Founderの要望を受け、直前の改訂（最低3つの情報源を確認）だけでは
    早期の打ち切りを防ぎきれないと判断し、2点を強化した。(1)5件に
    届くまで探索を続けることを原則とし、切り上げてよいのは質・停止
    条件で見送った場合か確認できる情報源を確認し尽くした場合に限る
    と明記、目安の情報源数も3→5へ引き上げた。(2)完了報告に、追加
    件数が5件未満の場合は確認した情報源数・一覧と5件に届かなかった
    具体的理由を明記する項目を追加し、「確認した情報源が5未満のまま
    見つからなかった」ことは正当な理由にならないと明記した（週次
    品質改善タスクの「20件未満で終了する正当な理由を明記する」と
    同型のパターンを踏襲）。品質基準・採用条件・停止条件は変更して
    いない。
```
