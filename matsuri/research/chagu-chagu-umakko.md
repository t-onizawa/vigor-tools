# チャグチャグ馬コ 調査記録

- 確認日: 2026-08-19
- 判定: 掲載（2026年終了）

## 範囲・同一性・開催主体

チャグチャグ馬コ保存会が案内する、鬼越蒼前神社から盛岡八幡宮までの行進行事を対象とする。前祝祭や関連ツアーは含めない。保存会は盛岡市・滝沢市・矢巾町等で構成され、1ページの範囲は明確。

## 2026年・features・差異

保存会・岩手県公式で2026年6月13日を確認し、終了済みのため `ended`。馬の行進のみを `hasParade: true` とし、山車・神輿・山車上の踊りは該当なし。既存祭りと異なる装束馬・鈴の音・約14kmの行進が核。highlightCommentは保存会公式説明から作成した。

## 素材

素材探索は行わず `backgroundImage: null`、`atmosphereMedia: []`。

## 一次情報

- https://chaguuma.com/
- https://chaguuma.com/news/%E4%BB%A4%E5%92%8C%EF%BC%98%E5%B9%B4%EF%BC%96%E6%9C%8813%E6%97%A5%EF%BC%88%E5%9C%9F%EF%BC%89%E3%83%81%E3%83%A3%E3%82%B0%E3%83%81%E3%83%A3%E3%82%B0%E9%A6%AC%E3%82%B3%E9%96%8B%E5%82%AC%EF%BC%81/
- https://www.pref.iwate.jp/event/moyooshi/1099317.html

## backgroundImage・atmosphereMedia調査（2026-09-01）

自治体・観光協会の公式情報を確認したが、直接確認可能な採用候補を特定できず、両素材とも見送り。
