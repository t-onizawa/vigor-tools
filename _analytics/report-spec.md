...
GA4・Search Consoleのデータ取得は、以下の優先順位で行う。

1. `python3 _analytics/scripts/fetch_analytics.py --start-date {開始日}
   --end-date {終了日}`を実行し、出力されたJSONを読み取る
2. スクリプト全体が実行できない場合（Pythonエラー、認証エラー等）、
   または個別カテゴリの`error`が`null`でない場合は、そのカテゴリに
   限り本書の各セクションに記載された従来手順（GA4管理画面・
   Search Console画面からの取得）にフォールバックする
3. どちらの方法でも取得できなかった項目は、無理に埋めず、Data Notes
   に「未取得・理由」を記録する

取得できる項目・集計ロジックは、本書の「GA4取得項目」「Search
Console取得項目」に記載の内容と同一とする。スクリプトから取得できる
からという理由で、記載のない指標を追加しない。
...
1. `fetch_analytics.py`の`ga4_tools`が正常に取得できていれば、それを
   使う（Organic流入、表示回数、上位ページの3項目。初反応ページは
   後述の方法で別途算出する）
2. スクリプトが失敗した場合、GA4に「TOOLS週次」Explorationが存在
   すればそれを開いて4項目を読み取る
3. どちらも利用できない場合、GA4の標準レポート（「エンゲージメント」
   →「ページとスクリーン」等）でページパスを`/tools/`に絞り込める
   範囲で代替取得する
4. 上記いずれの方法でも取得できなかった項目は、無理に埋めず、
   Data Notesに「未取得・理由」を記録する
