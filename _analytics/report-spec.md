...
（2026-09-17〜09-24：ChatGPT側のGSC Wizardプラグインを優先取得元と
する運用を試験導入したが、本レポートを実行するCodex Cloud Scheduled
Taskの環境からは呼び出せないことが実行結果で確認されたため終了した。
GSC WizardはFounder MTG側での深掘り分析用として別途利用する。）
