...
## データ取得方法（2026-09-17更新：GSC Wizard対応）

GSC・GA4のデータ取得は、以下の優先順位で行う。上位の方法が実行環境で
利用できない場合のみ、下位の方法にフォールバックする。

1. **GSC Wizard**（ChatGPTに導入済みのプラグイン。Google Search
   Console `sc-domain:vigorlab.net`とGA4「VIGOR LAB」を直接取得でき、
   ランディングページ単位でのGSC×GA4横断取得も可能）
2. 利用できない場合、GA4管理画面・Search Console画面からの取得
   （本書の各セクションに記載された従来手順）

**重要：GSC Wizardは元々ChatGPTの会話で動作確認されたものであり、
本レポートを生成する実行環境（Scheduled Task）から同じプラグインを
呼び出せるかは未確認である。** 実行環境が異なれば利用できない可能性が
ある。そのため、毎回の実行で実際にどちらの方法を使えたかを
Data Notesに明記すること（例：「GSC Wizard経由で取得」「GSC Wizard
利用不可のため従来方式で取得」）。これにより、数回の実行結果を見て
実際に利用可能かどうかを判断する。

GSC Wizardを使う場合も、以下は変更しない。

- Web検索と動画検索は分けて取得し、合算しない（MATSURIの合算値は
  従来通り別途算出する）
- GA4内部トラフィック除外ルールは変更しない。除外フィルタの状態
  （有効／テスト）はこれまで通りData Notesに記載する
- GSC Wizardで取得できる項目が増えても、本書に記載のない指標は
  追加しない。取得項目は既存の「GA4取得項目」「Search Console取得
  項目」のリストに限定する
...
取得方法は「データ取得方法」の優先順位に従う。GSC Wizardが利用できる
場合は、ランディングページを`^/matsuri/`で絞り込んだGSC×GA4横断取得を
優先する（TOOLSの「毎週の取得手順」と同じ考え方）。

## Search Console取得項目

取得方法は「データ取得方法」の優先順位に従う（GSC Wizard優先、
利用できない場合はSearch Console画面から取得）。
...
必須条件ではない。「データ取得方法」の優先順位に従い、以下の順で
取得する。

1. **GSC Wizardが利用できる場合**、ランディングページをページパス
   `^/tools/`で絞り込み、GSC×GA4横断取得で4項目（Organic流入、
   表示回数、上位ページ、初反応ページ）を取得する
2. GSC Wizardが利用できない場合、GA4に「TOOLS週次」Explorationが
   存在すればそれを開いて4項目を読み取る
3. どちらも利用できない場合、GA4の標準レポート（「エンゲージメント」→
   「ページとスクリーン」等）でページパスを`/tools/`に絞り込める範囲で
   代替取得する。標準レポートの絞り込みで4項目のうち取得できるものだけ
   埋める
4. 上記いずれの方法でも取得できなかった項目は、無理に埋めず、
   Data Notesに「未取得・理由（GSC Wizard利用不可／Explorationなし／
   標準レポートで絞り込み不可 等）」を記録する
5. 初反応ページの比較（前週の`_analytics/weekly/{前回日付}.md`との
   突き合わせ）は、取得元がどの方法であっても同じロジックで行う
